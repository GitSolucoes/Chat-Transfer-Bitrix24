...
def deal_updated():
    try:
        ...
        if deal_id:
            ...
            deal_data = requests.get(f"{base_url}/crm.deal.get?ID={deal_id}").json()

            if "result" in deal_data:
                responsible_id = deal_data["result"].get("ASSIGNED_BY_ID")
                contact_id = deal_data["result"].get("CONTACT_ID")

                if responsible_id and contact_id:
                    responsible_id = ''.join(filter(str.isdigit, str(responsible_id)))
                    requests.post(
                        f"http://localhost:1470/change-the-chat-responsible/?CONTACT_ID={contact_id}&TRANSFER_ID={responsible_id}"
                    )
...
def change_the_chat_responsability():
    ...
    TRANSFER_ID = ''.join(filter(str.isdigit, str(TRANSFER_ID)))
